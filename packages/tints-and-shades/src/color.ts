...
const toChannelHex = (value: number): string =>
  clampChannel(value).toString(16).padStart(2, "0");

const normalizeHue = (value: number): number =>
  ((value % 360) + 360) % 360;

const assertNormalizedHex = (value: string): string => {
...
export const normalizeHex = (value: string): string | null => {
  ...
  if (HEX_6_RE.test(stripped)) return stripped;
...
export const hexToRgb = (colorValue: string): RGB => {
...
export const rgbToHex = (rgb: RGB): string =>
  toChannelHex(rgb.red) + toChannelHex(rgb.green) + toChannelHex(rgb.blue);
...
const rotateHueHexes = (colorValue: string, offsets: number[]): string[] => {
  const normalized = assertNormalizedHex(colorValue);
...
export const getComplementaryHex = (colorValue: string): string =>
  rotateHueHexes(colorValue, [180])[0];

export const getSplitComplementaryHexes = (colorValue: string): [string, string] =>
  rotateHueHexes(colorValue, [150, 210]) as [string, string];

export const getAnalogousHexes = (colorValue: string): [string, string] =>
  rotateHueHexes(colorValue, [-30, 30]) as [string, string];

export const getTriadicHexes = (colorValue: string): [string, string] =>
  rotateHueHexes(colorValue, [120, 240]) as [string, string];
